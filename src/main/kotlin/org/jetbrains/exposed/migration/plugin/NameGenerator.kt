package org.jetbrains.org.jetbrains.exposed.migration.plugin
...
fun String.statementToFileName(): String {
    ...
    // Extract operation type and table name based on the SQL statement type
    return when {
        ...
        // Default case for other statements
            ...
            // Extract the first word as the operation
            val operation = normalizedStatement.split(" ").first().uppercase()

            // Try to find a table name if possible
            val tableName = tryExtractAnyTableName(normalizedStatement)

            if (tableName.isNotEmpty()) {
                "${operation}_$tableName"
            } else {
                // Use a hash of the statement if we can't extract meaningful info
                val hash = normalizedStatement.hashCode().toString().replace("-", "").take(8)
